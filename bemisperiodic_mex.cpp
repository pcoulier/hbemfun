/*BEMISPERIODIC   True for a periodic boundary element mesh.
 *
 *   periodic=BEMISPERIODIC(elt,typ) checks if the boundary element problem is
 *   periodic. If the mesh contains both periodic and non-periodic
 *   elements, an error is thrown. 
 *   
 *   elt    Elements.        
 *   typ    Element types.
 *   periodic True for periodic geometry, false otherwise.
 */

/* $Make: mex -O -output bemisperiodic bemisperiodic_mex.cpp bemisperiodic.cpp eltdef.cpp$*/
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
  try
  {
    ...
    const unsigned int probGeom = isPeriodic(Elt,nElt,TypeID,TypeName,TypeKeyOpts,nKeyOpt,nEltType);
