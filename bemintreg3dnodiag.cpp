...
void bemintreg3dnodiag(
				 ...
				 // const unsigned int* const EltShapeN, const int* const EltShapeM, 
...
//  mexPrintf("Nuniquescolli[0]: %u \n",Nuniquescolli[0]);
  ...
  if (spassed)
  {
	...
	// int* const inddiag=new(nothrow) int[9];
	 ...
	 //mexPrintf("uniquescolli[iuniquescolli]: %u \n",uniquescolli[iuniquescolli]);
	...
	// URe[0]=10; 
		...
		for (unsigned int iEltColl=0; iEltColl<nEltColl[iElt]; iEltColl++)
        {
			...
			if (blocks[iuniquescolli]==true && ms>1)
			{
				...
				unsigned int rowBeg=3*iuniquescolli;  // welke rijpositie -> sColi
				// // // unsigned int colBeg=3*EltCollIndex[iEltColl]-3*(NEltCollConsider+nEltCollConsider); 
				unsigned int colBeg=3*EltCollIndex[iEltColl]-3*(DeltaInListuniquecollj[EltCollIndex[iEltColl]]);  // !! Nodig voor NodalColl!!1
				...
				for (unsigned int iGrSet=0; iGrSet<nGrSet; iGrSet++)
				{
					...
					// // {
					 ...
					 // // unsigned int test=nDof;
					 // // mexPrintf("[ind0+ms*(colBeg+0)+rowBeg+0]: %u \n",ind0+ms*(colBeg+0)+rowBeg+0);
            		 // // mexPrintf("[ind0+ms*(colBeg+1)+rowBeg+0]: %u \n",ind0+ms*(colBeg+1)+rowBeg+0);
					 // // mexPrintf("[ind0+ms*(colBeg+2)+rowBeg+0]: %u \n",ind0+ms*(colBeg+2)+rowBeg+0);
					 // // mexPrintf("[ind0+ms*(colBeg+0)+rowBeg+1]: %u \n",ind0+ms*(colBeg+0)+rowBeg+1);
					 // // mexPrintf("[ind0+ms*(colBeg+1)+rowBeg+1]: %u \n",ind0+ms*(colBeg+1)+rowBeg+1);
					 // // mexPrintf("[ind0+ms*(colBeg+2)+rowBeg+1]: %u \n",ind0+ms*(colBeg+2)+rowBeg+1);
            		 // // mexPrintf("[ind0+ms*(colBeg+0)+rowBeg+2]: %u \n",ind0+ms*(colBeg+0)+rowBeg+2);
            		 // // mexPrintf("[ind0+ms*(colBeg+1)+rowBeg+2]: %u \n",ind0+ms*(colBeg+1)+rowBeg+2);
					 // // mexPrintf("[ind0+ms*(colBeg+2)+rowBeg+2]: %u \n",ind0+ms*(colBeg+2)+rowBeg+2);
	                // // }
