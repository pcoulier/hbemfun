...
//======================================================================
// AXISYMMETRIC BOUNDARY ELEMENT INTEGRATION
//======================================================================
void bemxferaxi(const double* const Nod,const unsigned int& nNod,
                ...
                const bool& ugCmplx, const bool& tgCmplx)
{
  ...
  //   nAzimutDiv sub-elements are considered in the azimuthal direction.
  ...
  unsigned int nAzimDiv=(unsigned int)(3.141592653589793*RadiusAverage/eLen);  // ONLY HALF CYLINDER, 0>PI
  if (nAzimDiv<1) nAzimDiv=1;
  const unsigned int nXi1=nGauss*nEltDiv;         // Number of Radial points.
  unsigned int nXi2=nAzimDiv*nGauss*nEltDiv;      // Number of Azimuthal points, minimum nGauss.
  ...
  // Non-linear mapping for theta (cfr. Book Dominguez):  REQUIRED FOR BEMXFER???
  //    theta = pi/4*(1 + xi)^2;
  ...
  for (unsigned int iXi2=0;iXi2<nXi2;iXi2++) xi2theta[iXi2]=1.570796326794897*(1.0+xi2[iXi2]);  // Regularly distributed points
  ...
  for (unsigned int iXi1=0; iXi1<nXi1; iXi1++)  // Radial direction
  {
    ...
    for (unsigned int iXi2=0; iXi2<nXi2; iXi2++) // Azimuthal direction
    {
      ...
      //Jac[iXi]= Jac1[iXi1]*xi1r*1.57079632679490*(1.0+xi2[iXi2]); // For dominguez procedure
      Jac[iXi]= xi1r*Jac1[iXi1]*1.57079632679490;
    }
  }
  ...
  // Initialize interpolation of Green's function (same as in 3D routine)
  unsigned int r1=0;
  unsigned int r2=1;
  ...
  for (unsigned int iRec=0; iRec<nRec; iRec++)
  {
    if (!(boundaryRec[iRec]))  // If receiver not on interface
    {
      for (unsigned int iXi=0; iXi<nXi; iXi++)
      {
        ...
        const double xiZ=xiCart[3*iXi+2]-Rec[2*nRec+iRec];
        ...
        greeneval3d(greenPtr,nGrSet,ugCmplx,tgCmplx,tg0Cmplx,xiR,xiZ,r1,r2,
                    ...
                    iRec,2,UgrRe,UgrIm,TgrRe,TgrIm,Tgr0Re,Tgr0Im);
        ...
        for (unsigned int iEltColl=0; iEltColl<nEltColl; iEltColl++)
        {
          ...
          for (unsigned int iGrSet=0; iGrSet<nGrSet; iGrSet++)
          {
            ...
            URe[ind0+nRecDof*(colBeg+0)+rowBeg+0]+=2.0*sumutil*(costheta*UXiRe[9*iGrSet+0]+sintheta*UXiRe[9*iGrSet+1]);
            URe[ind0+nRecDof*(colBeg+1)+rowBeg+0]+=2.0*sumutil*(UXiRe[9*iGrSet+2]);
            URe[ind0+nRecDof*(colBeg+0)+rowBeg+1]+=2.0*sumutil*(costheta*UXiRe[9*iGrSet+6]+sintheta*UXiRe[9*iGrSet+7]);
            URe[ind0+nRecDof*(colBeg+1)+rowBeg+1]+=2.0*sumutil*(UXiRe[9*iGrSet+8]);
            if (ugCmplx)
            {
              UIm[ind0+nRecDof*(colBeg+0)+rowBeg+0]+=2.0*sumutil*(costheta*UXiIm[9*iGrSet+0]+sintheta*UXiIm[9*iGrSet+1]);
              UIm[ind0+nRecDof*(colBeg+1)+rowBeg+0]+=2.0*sumutil*(UXiIm[9*iGrSet+2]);
              UIm[ind0+nRecDof*(colBeg+0)+rowBeg+1]+=2.0*sumutil*(costheta*UXiIm[9*iGrSet+6]+sintheta*UXiIm[9*iGrSet+7]);
              UIm[ind0+nRecDof*(colBeg+1)+rowBeg+1]+=2.0*sumutil*(UXiIm[9*iGrSet+8]);
            }
            if (TmatOut)
            {
              TRe[ind0+nRecDof*(colBeg+0)+rowBeg+0]+=2.0*sumutil*(costheta*TXiRe[9*iGrSet+0]+sintheta*TXiRe[9*iGrSet+1]);
              TRe[ind0+nRecDof*(colBeg+1)+rowBeg+0]+=2.0*sumutil*(TXiRe[9*iGrSet+2]);
              TRe[ind0+nRecDof*(colBeg+0)+rowBeg+1]+=2.0*sumutil*(costheta*TXiRe[9*iGrSet+6]+sintheta*TXiRe[9*iGrSet+7]);
              TRe[ind0+nRecDof*(colBeg+1)+rowBeg+1]+=2.0*sumutil*(TXiRe[9*iGrSet+8]);
              if (tgCmplx)
              {
                TIm[ind0+nRecDof*(colBeg+0)+rowBeg+0]+=2.0*sumutil*(costheta*TXiIm[9*iGrSet+0]+sintheta*TXiIm[9*iGrSet+1]);
                TIm[ind0+nRecDof*(colBeg+1)+rowBeg+0]+=2.0*sumutil*(TXiIm[9*iGrSet+2]);
                TIm[ind0+nRecDof*(colBeg+0)+rowBeg+1]+=2.0*sumutil*(costheta*TXiIm[9*iGrSet+6]+sintheta*TXiIm[9*iGrSet+7]);
                TIm[ind0+nRecDof*(colBeg+1)+rowBeg+1]+=2.0*sumutil*(TXiIm[9*iGrSet+8]);
              }
            }
          }
        }
