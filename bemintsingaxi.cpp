...
//============================================================================//
//  AXISYMMETRIC SINGULAR INTEGRATION
//============================================================================//
                   ...
                   const bool& UmatOut,const bool& TmatOut)
{
  ...
  double RadiusAverage=0.50*(EltNod[0*nEltNod+0]+EltNod[0*nEltNod+nEltNod-1]);
  unsigned int nAzimDiv=(unsigned int)(6.28318530717959*RadiusAverage/eLen);
  const unsigned int nXi1=nEltDivSing*nGaussSing; // Number of Radial points.
  unsigned int nXi2=nGaussSing;                   // Number of Azimuthal points.
  if ((eLen>0)&&(RadiusAverage>0)&&(nAzimDiv>nGaussSing)) nXi2=nAzimDiv;
  ...
  gausspw1D(nEltDivSing,nGaussSing,xi1,H1);
  // Integration points and weights for the axisymmetric element
  ...
  // Non-linear mapping for theta (cfr. Dominguez)
  //    theta = pi/4*(1 + xi)^2;
  ...
  for (unsigned int iXi1=0; iXi1<nXi1; iXi1++)  // Radial direction
  {
    ...
    for (unsigned int iXi2=0; iXi2<nXi2; iXi2++) // Azimuthal direction
    {
      ...
      Jac[iXi]=xi1r*Jac1[iXi1]*1.57079632679490*(1.0+xi2[iXi2]);
    }
  }
  ...
  // Initialise interpolation of Green's function (same as 3D routine)
  unsigned int r1=0;
  unsigned int r2=1;
  ...
  for (unsigned int iXi=0; iXi<nXi; iXi++)
  {
    ...
    // SUM UP RESULTS, FOR ALL COLLOCATION POINTS (different from 3D routine)
    for (unsigned int iEltColl=0; iEltColl<nEltColl; iEltColl++)
    {
      ...
      for (unsigned int iGrSet=0; iGrSet<nGrSet; iGrSet++)
      {
        ...
        URe[ind0+nDof*(colBeg+0)+rowBeg+0]+=2.0*sumutil*(costheta*UXiRe[9*iGrSet+0]+sintheta*UXiRe[9*iGrSet+1]);  // ugxr
        URe[ind0+nDof*(colBeg+1)+rowBeg+0]+=2.0*sumutil*(UXiRe[9*iGrSet+2]);                                      // ugxz
        URe[ind0+nDof*(colBeg+0)+rowBeg+1]+=2.0*sumutil*(costheta*UXiRe[9*iGrSet+6]+sintheta*UXiRe[9*iGrSet+7]);  // ugzr
        URe[ind0+nDof*(colBeg+1)+rowBeg+1]+=2.0*sumutil*(UXiRe[9*iGrSet+8]);                                      // ugzz
        if (ugCmplx)
        {
        ...
        if (TmatOut)
        {
          TRe[ind0+nDof*(colBeg+0)+rowBeg+0]+=2.0*sumutil*(costheta*TXiRe[9*iGrSet+0]+sintheta*TXiRe[9*iGrSet+1]); // tgxr
          TRe[ind0+nDof*(colBeg+1)+rowBeg+0]+=2.0*sumutil*(TXiRe[9*iGrSet+2]);                                     // tgxz
          TRe[ind0+nDof*(colBeg+0)+rowBeg+1]+=2.0*sumutil*(costheta*TXiRe[9*iGrSet+6]+sintheta*TXiRe[9*iGrSet+7]); // tgzr
          TRe[ind0+nDof*(colBeg+1)+rowBeg+1]+=2.0*sumutil*(TXiRe[9*iGrSet+8]);                                     // tgzz
          if (tgCmplx)
          {
