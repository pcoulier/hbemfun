				  ...
				  const double* const NodCoord)
{
  ...
  */		 
  const unsigned int nXi=nEltDivSing[iElt]*nEltDivSing[iElt]*nGaussSing[iElt]*nGaussSing[iElt];
  ...
  */
   ...
   for (unsigned int iDiv=0; iDiv<nDiv; iDiv++) if (rhom[iDiv]>1e-10)
  {
    ...
    for (unsigned int iXi=0; iXi<nXi; iXi++)
    {
      for (unsigned int iNod=0; iNod<nEltNod[iElt]; iNod++)
      {
...
// // 		const int ind0=ms*ns*iGrSet;
	  ...
	  else
	  {
		...
		double sumutil=rho[iXi]*H[iXi]*M[nEltColl[iElt]*iXi+iEltColl]*Jac[iXi]*0.25*(a2[iDiv]-a1[iDiv])*rhom[iDiv]/cos(a[iXi]-am[iDiv]);
		unsigned int rowBeg=3*iuniqueColl;
        // unsigned int colBeg=3*EltCollIndex[iEltColl]-3*(NEltCollConsider+nEltCollConsider);
		unsigned int colBeg=3*EltCollIndex[iEltColl]-3*(DeltaInListuniquecollj[EltCollIndex[iEltColl]]);
        ...
        double sumutil=rho[iXi]*H[iXi]*M[nEltColl[iElt]*iXi+iEltColl]*Jac[iXi]*0.25*(a2[iDiv]-a1[iDiv])*rhom[iDiv]/cos(a[iXi]-am[iDiv]);
			...
			for (unsigned int iuniquescolliind=0; iuniquescolliind<nuniquescolli[iuniqueColl]; iuniquescolliind++)
				{
...
//           mexPrintf("ind0: %d \n",ind0);
// // //           if (iDiv==0 && iXi==0 && iEltColl==0)
// // //           {
// // //           mexPrintf("iElt: %d \n",iElt);    
// // //           mexPrintf("rowBeg: %d \n",rowBeg);    
// // //           mexPrintf("colBeg: %d \n",colBeg);   
// // //           
// // //           mexPrintf("[ind0+test*(colBeg+0)+rowBeg+0]: %u \n",ind0+test*(colBeg+0)+rowBeg+0);
// // //           mexPrintf("[ind0+test*(colBeg+1)+rowBeg+0]: %u \n",ind0+test*(colBeg+1)+rowBeg+0);
// // //           mexPrintf("[ind0+test*(colBeg+2)+rowBeg+0]: %u \n",ind0+test*(colBeg+2)+rowBeg+0);
// // //           mexPrintf("[ind0+test*(colBeg+0)+rowBeg+1]: %u \n",ind0+test*(colBeg+0)+rowBeg+1);
// // //           mexPrintf("[ind0+nDof*(colBeg+1)+rowBeg+1]: %u \n",ind0+nDof*(colBeg+1)+rowBeg+1);
// // //           mexPrintf("[ind0+nDof*(colBeg+2)+rowBeg+1]: %u \n",ind0+nDof*(colBeg+2)+rowBeg+1);
// // //           mexPrintf("[ind0+nDof*(colBeg+0)+rowBeg+2]: %u \n",ind0+nDof*(colBeg+0)+rowBeg+2);
// // //           mexPrintf("[ind0+nDof*(colBeg+1)+rowBeg+2]: %u \n",ind0+nDof*(colBeg+1)+rowBeg+2);
// // //           mexPrintf("[ind0+nDof*(colBeg+2)+rowBeg+2]: %u \n",ind0+nDof*(colBeg+2)+rowBeg+2);
// // //           }
          URe[ind0+nDof*(colBeg+0)+rowBeg+0]+=sumutil*UXiRe[9*iGrSet+0];
          URe[ind0+nDof*(colBeg+1)+rowBeg+0]+=sumutil*UXiRe[9*iGrSet+1];
          URe[ind0+nDof*(colBeg+2)+rowBeg+0]+=sumutil*UXiRe[9*iGrSet+2];
