function [ds,cs,h]=bemgridplot_cyl(varargin)
%BEMGRIDPLOT_CYL   Plot results on a cylindrical receiver grid.
%
%   BEMGRIDPLOT_CYL(r,theta,z,u,c) creates a color plot of a scalar wave field 
%   c(r,theta,z) on a deformed mesh.  The mesh displacements are given by the 
%   vector field u that has been computed with the BEMXFER command for a 
%   receiver grid defined with the BEMGRID_CYL command.  The grid is defined 
%   in the cylindrical coordinate system and the deformation is defined in 
%   Cartesian coordinates.
%
%   r       Receiver grid r-coordinates (nr * 1).
%   theta   Receiver grid theta-coordinates (ntheta * 1).
%   z       Receiver grid y-coordinates (nz * 1).
...
%
%   BEMGRIDPLOT_CYL(...,ParamName,ParamValue) sets the value of the specified
...
%
%   [ds,cs,h] = BEMGRIDPLOT_CYL(...) returns the deformation scale ds, the color
...
if ~isempty(uRec)
  if size(uRec,1)==nRec
    ux=0;
    uy=reshape(uRec(1:1:nRec),[nrRec ntRec nzRec]);
    uz=0;
  elseif size(uRec,1)==2*nRec
    ux=reshape(uRec(1:2:2*nRec),[nrRec ntRec nzRec]);
    uy=0;
    uz=reshape(uRec(2:2:2*nRec),[nrRec ntRec nzRec]);
  elseif size(uRec,1)==3*nRec
    ux=reshape(uRec(1:3:3*nRec),[nrRec ntRec nzRec]);
    uy=reshape(uRec(2:3:3*nRec),[nrRec ntRec nzRec]);
    uz=reshape(uRec(3:3:3*nRec),[nrRec ntRec nzRec]);
  else
...
if ~isempty(cRec)
  c=permute(reshape(cRec(1:nRec),[ntRec nrRec nzRec]),[3 2 1]);
else
...
[ds,cs,h]=waveplot_cyl(rRec,tRec,zRec,ur,ut,uz,c,paramlist{:});
set(gca,'YDir','normal');
set(gca,'ZDir','normal');
...
%===============================================================================
function [ds,cs,h]=waveplot_cyl(varargin)
...
% CALL WAVEPLOT_REC
[ds,cs,h]=waveplot_rec(x,y,z,uxrec,uyrec,uzrec,c,paramlist{:});
